...
import { GEMINI_API_KEY, buildQuizPrompt, validateQuiz, getOrGenerateStudyContent } from '../../../server/studyContent.js'
...
export default async function handler(req, res) {
  ...
  try {
    const quiz = await getOrGenerateStudyContent(auth.db, note, 'quiz_json', {
    ...
    return res.status(500).json({ error: describeGeminiError(err) || 'Kviza ni bilo mogoče ustvariti. Poskusi znova.' })
  }
}
